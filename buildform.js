...
function updateGeoLocation() {
    if (geostatus.length > 0) {
        ...
        // #260/#265: persist the correction back into alldata's own entry
        // once the fresh query has actually completed (queryGeo() is
        // async - this only runs once geostatus confirms it's done) - see
        // persistGeoUpdateToSourceEntry()'s own comment for why this
        // matters, and why it no longer touches entry.location/entry.id.
        persistGeoUpdateToSourceEntry(googlerequery, locationdata);
...
// showing the stale original result, discarding the correction.
//
// #265 follow-up (live-reported): this function's first version also
// overwrote entry.location with whatever was just submitted, and
// repointed entry.id at the fresh geoid slot. That broke the pencil's
// own "Restore" button, which reads entry.location (via
// getParsedLocation()) as the TRUE, untouched original scraped text -
// once a genuine correction was typed and submitted, that original was
// permanently gone, since this was the only place it lived. Now leaves
// entry.location AND entry.id completely untouched forever - the fresh
// geo RESULT is written into the EXISTING geolocation[] slot that
// entry.id already points to, in place, so a later re-render sees it
// without redefining what "the original" means. The fresh geoid slot
// queryGeo() actually populated is simply left orphaned, harmless.
function persistGeoUpdateToSourceEntry(dataid, freshResult) {
    var entry = getParsedLocationEntry(dataid);
    if (!exists(entry) || !exists(entry.id)) {
        return;
    }
    geolocation[entry.id] = freshResult;
}
