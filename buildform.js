    ...
    //        }
        ...
        for (var list in listvalues) if (listvalues.hasOwnProperty(list)) {
            ...
            if (exists(obj) && obj.length > 0) {
                ...
                if (!dateadded) {
                    membersstring = membersstring +
                        '<tr style="display: ' + isHidden(hidden) + ';" class="hiddenrow"><td class="profilediv"><input type="checkbox" class="checknext">' + capFL(title) + ' Date: </td><td style="float:right;"><input type="text" class="formtext dateform" name="' + title + ':date" disabled></td><td class="genisliderow"><img src="images/' + dateicon + '" class="genislideimage"><input type="text" class="formtext genislideinput" value="' + escapeHtml(genifocusdata.get(title, "date.formatted_date").replace(/&quot;/g, '"')) + '" disabled></td></tr>';
                }
                if (title === "death") {
